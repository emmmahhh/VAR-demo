...
def main():
    ...
    parser.add_argument('--output_dir', type=str, required=False, help="Output directory to save images",default="./outputs")
    ...
    # 本地权重路径（放在 pretrained_models 文件夹下）
    vae_ckpt = 'pretrained_models/vae_ch160v4096z32.pth'
    var_ckpt = f'pretrained_models/var_d{MODEL_DEPTH}.pth'

    # 检查文件是否存在（如果不存在，提示用户手动下载）
    if not osp.exists(vae_ckpt):
        raise FileNotFoundError(f"VAE checkpoint not found at {vae_ckpt}. Please download it and place in pretrained_models/")
    if not osp.exists(var_ckpt):
        raise FileNotFoundError(f"VAR checkpoint not found at {var_ckpt}. Please download it and place in pretrained_models/")
    ...
    save_folder = os.path.join(output_dir, "VAR-256-ariter{}-diffsteps{}-cfg{}-image{}".format(num_iter, num_sampling_steps, cfg, num_images))
